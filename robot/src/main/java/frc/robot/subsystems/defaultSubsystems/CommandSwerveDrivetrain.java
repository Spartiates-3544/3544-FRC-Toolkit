...
public class CommandSwerveDrivetrain extends TunerSwerveDrivetrain implements Subsystem {
     ...
     *                              The pose of the robot as measured by the vision
     *                              camera.
     * @param timestampSeconds
     *                              The timestamp of the vision measurement in
     *                              seconds.
     */
    @Override
     ...
     *
     * @param visionRobotPoseMeters
     *                                 The pose of the robot as measured by the
     *                                 vision camera.
     * @param timestampSeconds
     *                                 The timestamp of the vision measurement in
     *                                 seconds.
     * @param visionMeasurementStdDevs
     *                                 Standard deviations of the vision pose
     *                                 measurement
     *                                 in the form [x, y, theta]ᵀ, with units in
     *                                 meters and radians.
     */
    ...
    public void configAutoBuilder() {
        RobotConfig robotConfig = Constants.Drive.config;
        if (robotConfig == null) {
            Path settingsPath = Filesystem.getDeployDirectory().toPath().resolve("pathplanner/settings.json");
            if (!Files.exists(settingsPath)) {
                DriverStation.reportWarning(
                        "Skipping PathPlanner AutoBuilder configuration because deploy/pathplanner/settings.json is missing.",
                        false);
                return;
            }
            ...
            try {
                ...
                Constants.Drive.config = robotConfig;
            } catch (Exception e) {
                DriverStation.reportWarning(
                        "Skipping PathPlanner AutoBuilder configuration because deploy/pathplanner/settings.json could not be loaded.",
                        false);
                return;
            }
        }
        ...
        if (robotConfig == null) {
            DriverStation.reportWarning(
                    "Skipping PathPlanner AutoBuilder configuration because PathPlanner returned no robot config.",
                    false);
            return;
        }
